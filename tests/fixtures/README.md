...
Fixtures are added incrementally as each task in `tasks.md` needs them — this directory starts empty except for this file. Tests that need to simulate the *shape* of a live TMDB response without a static file (e.g., pagination behavior across multiple pages) may instead build the response inline via `httpx.MockTransport` — that's still "no live network call," just generated in-test rather than loaded from a file. Use a file under `tmdb/` when the exact recorded content matters (e.g., a specific overview for prompt-injection testing); use a `MockTransport` when only the response *shape* matters (e.g., pagination, HTTP status codes).
