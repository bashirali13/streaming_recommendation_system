...
- **RelaxableConstraint**: `runtime` | `year_range` — the closed set FR-011 permits the Orchestrator to relax, in that fixed priority order. No other field is ever a legal value here, which is what makes "hard constraints are never relaxed" (FR-010) mechanically enforceable rather than a convention.
...
| `vibe_keywords` | `list[str]` | From `PreferenceProfile.setting_descriptors + theme_descriptors` -- concrete subject matter such as "heist" or "winter". `tone_descriptors` are deliberately excluded (T111): TMDB's keyword tagging is too sparse for mood words ("sweet" is on 27 movies in its whole catalog), so requiring them wiped out valid answers; mood is used for ranking instead. Resolved to TMDB keyword ids inside `RealTmdbClient.discover()` via `/search/keyword`. Soft: an unresolved descriptor is dropped. Narrow first (every id, then any id); if that leaves fewer than 10 results the keyword filter is dropped and the narrow matches are kept at the front of the pool -- a query-construction detail, not a disclosed relaxation |
