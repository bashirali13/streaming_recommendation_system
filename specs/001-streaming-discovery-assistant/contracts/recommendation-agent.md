...
- MUST score remaining candidates against soft preferences (tone/setting/theme descriptors, liked/disliked titles, recency) and produce a ranked order (FR-014). MUST give a candidate satisfying every stated `theme_descriptors` term a scoring bonus large enough to reliably outrank a candidate satisfying only some of them, even under a realistic `vote_average` disadvantage (T102) — otherwise T100/T101's ability to actually discover a full-theme match via keyword search is wasted, since a flat per-hit bonus alone let a more popular partial match keep winning the slot instead. Scoped to `theme_descriptors` only, for the same reason the T092 relevance floor is theme-only: theme is the concrete, named-thing-the-user-wants signal, so tone/setting completeness is never required to win this bonus.
