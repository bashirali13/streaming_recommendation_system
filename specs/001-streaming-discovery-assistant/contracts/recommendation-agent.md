...
- MUST re-apply all hard filters (format, requested genres, year range, streaming service, movie runtime, excluded genres, `excluded_keywords`, any `hard_override_fields`) before any soft scoring, even though the Discovery Agent already filtered (T112: TMDB's own filters can disagree with a title's details -- a 141-minute pick for "under 2 hours" -- and the similar-titles path applies none; only a constraint the Orchestrator explicitly relaxed is skipped) — this is the ranking-side half of FR-009, protecting against a validation gap upstream rather than duplicating trust in it. For `excluded_keywords` specifically (T091), this re-check also covers `thematic_keywords` — enrichment data this agent is the first to see, so it can catch an exclusion the Discovery Agent's own (title/overview-only, pre-enrichment) check couldn't.
...
- MAY make one batched model call scoring how well the best-ranked candidates (up to 24) fit the requested mood, from only each candidate's own genres, keywords and overview (T116). The score is a ranking signal within a theme-completeness tier: it MUST NOT remove a candidate, MUST be skipped when there is no stated mood or three or fewer candidates, and a failed call MUST leave the deterministic ranking intact. This exists because TMDB's keyword tagging is too sparse to filter on mood words (T111), so mood can only be judged from a candidate's own details.
