...
- MUST, when the query names liked titles AND states facts the similar-titles list cannot honor (provider, year range, runtime, language), also run a filtered discover query -- using the user's genres, or the seed's own -- and add its results after the similar titles (T115). TMDB's `/similar` list applies no filters, so a stated fact must not depend on it.
