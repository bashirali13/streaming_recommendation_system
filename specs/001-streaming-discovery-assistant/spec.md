...
- **FR-011**: When the initial discovery search yields zero qualifying candidates, the system MUST perform exactly one additional discovery attempt with exactly one pre-approved soft constraint relaxed, chosen in this fixed order of preference: (1) runtime ceiling, (2) release-year range. Subjective mood/tone words are never sent to TMDB as filters (T111), so there is nothing to relax for them, and a stated genre is never relaxed.
