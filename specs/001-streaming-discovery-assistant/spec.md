...
- **PreferenceProfile**: The structured, validated representation of what the user wants — media type, providers, genres and excluded genres, tone/setting/theme descriptors, languages, release-year and runtime bounds, a TV season-count cap (User Story 5; always hard when the user states one), liked/disliked titles, and an explicit separation of hard constraints from soft preferences.
- **DiscoveryQuery**: The concrete, TMDB-queryable request the Orchestrator derives from a PreferenceProfile for one discovery attempt — media type, provider names, configured region, included/excluded genres, setting/theme descriptors as `vibe_keywords` (T087; resolved to real TMDB keyword ids inside the TMDB adapter, not interpreted), year range, runtime bound (where the target media type supports server-side filtering), language, similarity-seed titles, which single constraint (if any) is relaxed for this attempt, and the retry number. It still excludes `additional_notes`, free-text correction text, and (T089) `tone_descriptors` — tone is a fuzzy mood signal rather than the concrete subject matter TMDB's keyword catalog represents, and mixing it into the discovery-side filter let an unrelated candidate through on a noisy tone match alone; tone stays a Recommendation Agent scoring/rationale signal only.
