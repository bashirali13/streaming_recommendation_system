...
### A deeper look: a "not Marvel or DC" request produced a Marvel title as Best Match and an unrelated romance as Wildcard Pick

A live run ("Superhero movie, Not Marvel or DC") surfaced not one bug but a real architectural gap, traced in full during this conversation: `additional_notes` (where the Marvel/DC exclusion landed, since it isn't a TMDB genre) is captured by the Preference Agent but was never read by anything downstream -- not the hard filter, not soft scoring, not even the rationale prompt. Separately, T087's `vibe_keywords` ORs tone descriptors together with theme/setting descriptors in the discovery-side keyword filter, so a fuzzy tone word ("hopeful") let a thematically unrelated candidate (a non-superhero romance) into the pool. And even once a bad candidate is in the pool, `RecommendationAgent`'s selection has no relevance floor -- it ranks by a formula and always fills up to 3 slots, regardless of how low the best score is, contradicting spec.md's own Clarification Q1 ("the system does not... accept lower-quality matches merely to fill an empty role"). Four fixes, in dependency order:

- [x] T089 Narrow `DiscoveryQuery.vibe_keywords` (T087) to `setting_descriptors` + `theme_descriptors` only, dropping `tone_descriptors` -- tone is a fuzzy/subjective mood signal, not the concrete kind of subject matter TMDB's keyword catalog is built around, and OR-ing it in let an irrelevant candidate satisfy the filter via a noisy tone-keyword match alone. Tone stays fully in play for the Recommendation Agent's soft scoring/rationale (unchanged); it's removed only from the discovery-side hard keyword filter.
- [x] T090 Wire `PreferenceProfile.additional_notes` into `RecommendationAgent.build_rationale_prompt` (it was documented as "consumed only by the Recommendation Agent's rationale step" since Foundational but never actually reached that prompt -- the same class of documented-but-unwired gap as `UserSessionState`/`Settings.demo_mode` before Polish fixed those). Lets the model's own rationale at least narrate a stated exclusion it's now aware of, ahead of T091 giving it real enforcement.
- [x] T091 Add `excluded_keywords` (`PreferenceProfile` -> `DiscoveryQuery`) for exclusions that don't fit `excluded_genres`'s fixed TMDB-genre vocabulary (franchises, studios, character universes, etc. -- e.g. "not Marvel or DC"), hard by default per the Preference Agent's existing hard-constraint classification rule. Resolved server-side via TMDB's `without_keywords` (reusing T087's phrase/per-word keyword-search resolution), plus a defense-in-depth text-match re-check against title/overview in the Discovery Agent's bulk-item hard filter and again against title/overview/`thematic_keywords` in the Recommendation Agent's hard filter -- since TMDB's own keyword tagging isn't guaranteed complete for every excluded concept, the text-match layer is what actually upholds "never silently drop a hard constraint" (FR-010) when keyword resolution alone can't.
- [x] T092 Give `RecommendationAgent.run()` a relevance floor: when `theme_descriptors` are stated, a candidate with zero text-match hits against them is dropped from selection entirely, not merely ranked lower -- closing the gap where a thin, hard-filter-surviving pool got force-filled up to 3 picks regardless of relevance (a "hopeful, superhero" request's Wildcard Pick was a romance with zero superhero relevance, because nothing gated on it). **Narrowed during implementation**: the original plan also gated on `setting_descriptors`, but that broke `tests/e2e/test_vague_mood_request.py`, which encodes spec.md User Story 2's own acceptance criteria -- a vague, tone/setting-only request MUST still surface a weakly-evidenced pick with an honest `confidence_note`, not drop it. Only `theme_descriptors` gates (a starker "is this even the right kind of story" signal than setting ever is); `tone_descriptors`/`setting_descriptors` stay ranking/rationale-only, unchanged. Falls into the existing "No candidates satisfied your constraints" path if this empties the pool; no change to the Orchestrator's retry trigger (out of scope for this task -- noted as a possible follow-up, not folded in here).
...
### Second live regression on the same request class: "Superhero movie" (Marvel/DC exclusion again ignored; picks again irrelevant)

A second live run, after T089-T092, showed both original symptoms recurring. Root-caused to two things, one confirmed code bug and one prompt-reliability gap that turned out to be the shared cause of *both* symptoms:

- [x] T093 `PreferenceProfile.excluded_keywords` (T091) was never wired into either confirmation display -- `build_confirmation_summary` (orchestrator.py) and `_SUMMARY_FIELDS` (cli/rich_ui.py) both lacked a row for it. Confirmed code bug: even when captured, the user had no way to see or correct it before the search ran. Fixed in both; tests written first (`tests/unit/test_confirmation.py`, `tests/unit/test_rich_ui.py`), confirmed red, then made to pass.
- [x] T094 The Preference Agent's system prompt told the model that a genre-like term not in the fixed TMDB list belongs in `theme_descriptors` only by calling it a "descriptive mood or vibe" -- but a subject/content-category word like "superhero" isn't a mood, so it landed nowhere: not `genres` (correctly, since it isn't an official genre), and not `theme_descriptors` either. This explains *both* symptoms at once: the ignored exclusion and the irrelevant picks share the same root cause, since T092's relevance floor gates on `theme_descriptors` and had nothing to gate on when that field came back empty -- T092 would have worked correctly here if extraction had succeeded. Rewrote the system prompt with an explicit rule ("whatever the user says the content should be ABOUT must end up SOMEWHERE, never silently dropped") plus concrete worked examples for both the theme-classification gap ("superhero movie" -> `theme_descriptors: ["superhero"]`) and the exclusion gap ("not Marvel or DC" -> `excluded_keywords: ["Marvel", "DC"]`), since LLMs follow concrete examples more reliably than abstract rules for this kind of edge-case classification. Not deterministically testable (no live model in the test suite) -- T093 is what makes a remaining extraction miss visible and user-correctable rather than silent, which is the structural half of this fix that doesn't depend on the prompt working.
- [x] T095 A live re-verification of T091-T094 (run with the user's explicit go-ahead, using real TMDB/OpenRouter credentials) confirmed T093/T094 worked -- the confirmation table correctly showed `Excluding: Marvel, DC` and `Themes: superhero` -- but Spider-Man: Into the Spider-Verse still won Best Match despite the exclusion. Root-caused by fetching the film's real TMDB data directly: its real overview and real keyword tags (`["superhero", "based on comic", "aftercreditsstinger", "alternate universe"]`) never mention "Marvel" at all -- so neither T091's title/overview text-match nor TMDB's own `without_keywords` filter could ever have caught it -- while its real `production_companies` field does include "Marvel Entertainment". Added a third, post-detail() defensive check in the Discovery Agent against `production_companies` (data already fetched by the existing `details()` call, no new TMDB call needed), which is the one field TMDB reliably populates with the actual studio/publisher name for exactly this class of exclusion. Test written first (a fixture matching the real TMDB shape confirmed above -- no "Marvel" in title/overview/keywords, only in `production_companies`), confirmed red, then made to pass. Updated `contracts/discovery-agent.md`.
- [x] T096 The confirmation-correction prompt applied a non-blank correction (or failed to) with zero feedback either way, reported after a live run: typing "Hulu" (missing the `providers=` prefix) silently did nothing, with no indication the correction wasn't understood. Traced to `_parse_correction` returning `{}` for unrecognized input, and `{} or None` evaluating to `None` in `default_confirm` -- indistinguishable from a blank ("accept as-is") answer. A second, related crash risk found while fixing this: a correction that parses syntactically but produces an invalid profile (e.g. an inverted year range) called `apply_correction`, which raises `ContractValidationError` -- uncaught anywhere in `cli/intake.py`'s try/except, meaning it would have crashed the whole session rather than being a controlled failure. Fixed both in `default_confirm` (cli/output.py): an unrecognized (no `field=value` match) or invalid (fails `apply_correction`) correction now prints an explicit message and is treated as no-op, never silently dropped and never crashing; a correction that succeeds echoes the updated profile back (via the same `render_summary`/plain-text path used for the initial summary) before returning, so the user can see it actually took rather than silently carrying forward to discovery. Verified live in demo mode for both the unrecognized-correction and valid-correction paths. Tests written first (`tests/integration/test_confirmation_free_text.py`), confirmed red, then made to pass.
- [x] T097 A "family-friendly kids movie, humor, cartoon animation, classic, before 1970, avoid Looney Tunes" request returned "No candidates satisfied your constraints" despite real matches existing -- verified directly against live TMDB: the exact filter combination (Animation+Family genres AND'd, the four requested providers OR'd, before 1970) returns 45 real results (Sleeping Beauty, Snow White, Cinderella, Alice in Wonderland, 101 Dalmatians, ...) with no `with_keywords` filter, but adding `with_keywords=<classic's resolved TMDB keyword id>` -- exactly what `theme_descriptors: ["classic"]` produces via T087 -- collapses that same query to 0. Root cause: T087's keyword-search approach works well for concrete, nameable subject matter ("superhero", "heist"), which is what T094 correctly taught the model to route to `theme_descriptors`, but TMDB's keyword tagging essentially never applies a literal "classic" tag to old films -- it's a reputation/era descriptor, not subject matter, and T094's prompt didn't distinguish the two, so "classic" was landing in `theme_descriptors` and being used as a hard-ish discovery filter and (T092) a selection gate, exactly like providers/genres/exclusions are meant to be, when it should have stayed a purely soft, ranking-only signal like "dark"/"gritty"/"cozy" already correctly are. Extended the system prompt with an explicit theme-vs-tone distinction and worked examples (`classic`/`cult`/`iconic`/`nostalgic`/`acclaimed`/`feel-good` -> `tone_descriptors`, never `theme_descriptors`) -- since `tone_descriptors` is already excluded from both T087's discovery-side keyword filter (T089) and T092's relevance floor, no other code change was needed; getting the classification right was the whole fix. Verified live against the exact reported request. Raised by a pasted external analysis proposing a 6-level cascading relaxation policy and a "guaranteed non-empty results" design; both were declined as direct conflicts with this project's own deliberate, spec-documented principles (FR-011/NFR-007's exactly-one-bounded-retry; FR-010 and Clarification Q1's "never relax a hard constraint or accept a lower-quality match just to avoid an empty result") -- the external analysis's claimed root cause (an invalid literal genre filter) also didn't match this codebase's actual behavior once checked against the real confirmation table.
- [x] T098 Live-verifying T097 against the exact reported request found a third, related gap: "cartoon animation" landed entirely in `tone_descriptors` rather than being normalized to the official genre "Animation" (which IS in the fixed list), so the actual `with_genres` filter ended up looser than the request intended, and non-animated titles (*It's a Wonderful Life*, *The Sound of Music*) were recommended alongside a genuinely animated one (*Fantasia*) -- each rationale honestly flagged the animation mismatch itself, same pattern as T092/T097's own diagnosis. The system prompt's "only use one of these exact names" instruction was being read as "skip a genre word that isn't a verbatim match" rather than "normalize a close synonym to the matching official name" -- added an explicit normalization instruction with worked examples ("cartoon"/"cartoon animation"/"animated"/"anime" -> "Animation"; "rom-com" -> "Romance" + "Comedy"; "sci-fi" -> "Science Fiction"). Verified live.

### A 10-prompt live grading pass (user-authored, spanning specific to vague) surfaced a real rationale-generation bug

- [x] T099 Ran 10 varied live requests end-to-end and graded each (constraint fidelity, relevance, diversity). Two of ten showed the same bug: a role's displayed title and its written rationale describe two different titles entirely -- "Safe Pick: Re:ZERO..." with a rationale entirely about "Erased," and "Wildcard Pick: Reservoir Dogs" with a rationale entirely about "How to Steal a Million." Both occurred on the *weakest*-fitting candidate in their pool, suggesting the model substitutes a better-fitting title it knows from training data rather than honestly critiquing the one it was actually given -- `RATIONALE_SYSTEM_PROMPT` said "never invent a fact about the title" but never explicitly forbade substituting a different title outright. Fix: add a `for_title` field to `_RationaleOutput` (required, no default, so pydantic-ai's structured-output schema enforces it on the real model rather than leaving it optional) that the model must echo back verbatim; `RecommendationAgent._generate_rationale` deterministically compares it against the actual candidate's title and, on a mismatch, retries once with an explicit correction appended to the prompt, then falls back to a safe, generic, deterministic message (never showing a wrong-title rationale) if it still doesn't match -- converting an LLM reliability failure that's hard to detect in free text into one code can verify and bound, consistent with this project's "wording is the model's job, whether it succeeded is code's job" pattern used everywhere else. Required updating every existing test fixture that constructs `_RationaleOutput` directly to supply a matching `for_title`.

### Re-running the same 10 prompts confirmed T099 and surfaced two more root-level gaps, not covered by a quick patch

Re-running all 10 prompts confirmed the title-mismatch bug (T099) is gone in both previously-failing cases, and, notably, the empty-overview-candidate issue from the original grading pass (test #8) also no longer occurs -- traced to better theme extraction now giving T092's relevance floor something to gate on, incidentally filtering out data-empty candidates too. Two genuine gaps remained, both discussed with the user rather than patched individually:

- [ ] T100 Multiple `theme_descriptors`/`setting_descriptors` named together (e.g. "road trip and found family") were OR'd in TMDB's `with_keywords`, so a candidate satisfying only one of several named things could still win every slot -- confirmed live (found_family-only candidates in all 3 picks, road_trip in none). Fix: `RealTmdbClient.discover()` tries AND semantics (comma-joined) across all resolved `vibe_keywords` ids first; if that specific `discover()` call returns zero total results, it retries -- within that same call, invisibly, not consuming the Orchestrator's one disclosed bounded retry (FR-011) -- with OR semantics instead. TMDB's `with_keywords` only supports one separator per call, so this can't be a single mixed AND/OR expression; it has to be two sequential attempts.
- [ ] T101 Tone-driven or tone-contradicting-genre requests ("cozy heist," "rainy Sunday afternoon," pure-mood TV requests) never had tone influence *which* candidates got discovered at all -- T089 deliberately excluded `tone_descriptors` from the discovery-side keyword filter after it caused the original Marvel/DC-era false positive (an ungated OR match on a fuzzy tone word alone let an unrelated candidate through). Re-examining why that was unsafe: the danger wasn't "tone influences search," it was "tone was OR'd in as an equal substitute for a stated concrete theme, with nothing else required to hold." T100's AND-first logic changes that -- tone can now be folded back into the same `vibe_keywords` pool and combined via the same AND-first/OR-fallback mechanism, so tone can only ever *narrow* a search anchored by a real theme, never stand in for one. And even in the fallback-to-OR case, T092's relevance floor (built independently, after the original bug) still requires a genuine `theme_descriptors` match to be selectable at all whenever a theme was stated -- a second, structural layer that doesn't depend on how the candidate entered the pool. The only case without that safety net is a pure-tone-only request, where the worst outcome is an honestly-flagged weak match (already today's behavior for that case) rather than a wrongly-included excluded title -- and `excluded_keywords`/`excluded_genres` are a fully separate, unaffected enforcement path (T091/T095), which is specifically what makes this safe to do now and wasn't true when T087 first tried it. Fix: fold `tone_descriptors` back into `PreferenceProfile`/`Orchestrator.build_discovery_queries`'s `vibe_keywords` construction (reversing T089's field composition, for a materially different, now-safe reason), resolved through the same AND-first/OR-fallback mechanism as T100.
