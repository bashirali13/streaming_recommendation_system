...
- [ ] T092 Give `RecommendationAgent.run()` a relevance floor: when `theme_descriptors`/`setting_descriptors` are stated, a candidate with zero hits against them (via the existing `_descriptor_hit_count` text-match signal) is dropped from selection entirely, not merely ranked lower -- closing the gap where a thin, hard-filter-surviving pool got force-filled up to 3 picks regardless of relevance, contradicting spec.md Clarification Q1. Falls into the existing "No candidates satisfied your constraints" path if this empties the pool; no change to the Orchestrator's retry trigger (out of scope for this task -- noted as a possible follow-up, not folded in here).
