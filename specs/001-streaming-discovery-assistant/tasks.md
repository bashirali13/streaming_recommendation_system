...
- [ ] T022 [P] Define the `TmdbClient` `Protocol` (methods for discover/search, similar-title lookup, and detail fetch, each accepting a `result_limit`) in `src/streaming_discovery/tmdb/client.py`, shared by the real and fake implementations, per contracts/discovery-agent.md
- [ ] T023 [P] Unit test: given a fixture response spanning more pages/items than `result_limit`, the `TmdbClient` implementation stops paginating/collecting at the bound and never returns more than `result_limit` items, in `tests/tmdb_adapter/test_pagination_bound.py` (NFR-003) — write this first; it must fail before T024 exists
- [ ] T024 [P] Implement the real `httpx`-based `TmdbClient` in `src/streaming_discovery/tmdb/client.py`: request timeout and the bounded pagination/result limit from T023 enforced on every call (NFR-003)
- [ ] T025 [P] Implement `FakeTmdbClient` in `src/streaming_discovery/tmdb/fake_client.py`, reading recorded payloads from `tests/fixtures/tmdb/*.json`, honoring the same `result_limit` contract as the real client, and supporting an injectable failure mode (timeout / HTTP error / malformed response) and a call-count instrumentation hook (for later finalist-only-fetch verification) for adapter and integration tests (NFR-004)
- [ ] T026 [P] Implement `normalize.py` in `src/streaming_discovery/tmdb/normalize.py`: raw TMDB payload → `CandidateMedia`, resolving genre ids to names once (never carrying raw ids downstream), converting `release_date` to `release_year`, and scoping provider data to `Settings.region` and flatrate offers only (FR-019, FR-029)
- [ ] T027 [P] Adapter test: `normalize.py` output never contains a value corresponding to `popularity`, `vote_count`, raw genre ids, poster/backdrop paths, or a rent/buy provider offer, even when the input fixture contains them, in `tests/tmdb_adapter/test_normalize.py`
- [ ] T028 [P] Adapter test: `FakeTmdbClient` configured for timeout / HTTP error / malformed response yields a `CandidatePool` with `error` (a `TmdbErrorInfo`) set and `candidates=[]` — never a crash, never a fabricated candidate, in `tests/tmdb_adapter/test_fake_client_errors.py` (FR-027)

### LLM provider (ports + fakes)

- [ ] T029 [P] Define the `ModelProvider` `Protocol` in `src/streaming_discovery/llm/provider.py`, used only by the Preference Agent and Recommendation Agent (NFR-008)
- [ ] T030 [P] Implement a bounded-retry wrapper around `ModelProvider` calls in `src/streaming_discovery/llm/provider.py`: on an outright call failure (not a schema-validation failure), retry up to `Settings.llm_retry_max_attempts` additional times, then raise a controlled, typed error (FR-028)
- [ ] T031 [P] Implement `FakeModelProvider` in `src/streaming_discovery/llm/fake_provider.py`: returns fixture-backed deterministic responses from `tests/fixtures/llm/`, with an injectable "fail N times then succeed" / "always fail" mode for retry testing
- [ ] T032 [P] Contract test: the bounded-retry wrapper succeeds when the underlying call fails fewer times than `llm_retry_max_attempts`, and raises the controlled error when it fails more times than that, in `tests/contract/test_llm_retry.py` (FR-028)

### Orchestrator skeleton

- [ ] T033 Implement the `Orchestrator`'s session lifecycle and single-attempt sequencing (create `UserSessionState`, invoke Preference Agent, validate its output against `PreferenceProfile`, invoke Discovery Agent, validate its output against `CandidatePool`) in `src/streaming_discovery/agents/orchestrator.py`, per contracts/orchestrator.md — retry logic itself is added in Phase 6 (US4)
- [ ] T034 Implement the preference confirmation step as a logic-layer function (given a `PreferenceProfile`, produce a renderable summary; accept a correction payload and re-validate it against `PreferenceProfile`) in `src/streaming_discovery/agents/orchestrator.py` (FR-006). This is deliberately terminal-agnostic here — User Story 1 (T045) is what wires it to actual terminal I/O for every request path, not only guided intake; see the Dependencies section note on FR-006 scope.
- [ ] T035 [P] Integration test: an agent output that fails its contract's validation is surfaced as a controlled failure by the Orchestrator, never silently coerced or passed downstream, in `tests/integration/test_contract_validation_boundary.py` (FR-022, NFR-002)

**Checkpoint**: Foundation ready — every user story phase below builds on T006–T035.
...
- [ ] T036 [P] [US1] Contract test: the Preference Agent's output for the fixture specific-constraint request marks `media_type` and the excluded genre as hard, and the tone/recency signals as soft, in `tests/contract/test_preference_agent_us1.py` (spec.md US1 Acceptance Scenario 1)
- [ ] T037 [P] [US1] E2E test: the full pipeline for the specific-constraint request returns exactly three distinct titles assigned to Best Match/Safe Pick/Wildcard Pick, none in the excluded genre, with no raw TMDB JSON or raw inter-agent JSON in the rendered output, in `tests/e2e/test_specific_constraint_request.py`, using the quickstart.md scenario 1 fixture set (spec.md US1 Acceptance Scenarios 2–3)
- [ ] T038 [P] [US1] E2E test: given a fixture pool with exactly 1 qualifying candidate, only `best_match` is populated in the final output; given exactly 2, `best_match` and `safe_pick` are populated and `wildcard_pick` is `None`; in neither case is any constraint broadened or a candidate reused across roles, in `tests/e2e/test_partial_fill.py`, using the quickstart.md scenario 1b fixtures (FR-015, SC-008, resolved Clarification Q1)
- [ ] T039 [P] [US1] Integration test: for a free-text request (no guided intake involved), the CLI renders the `PreferenceProfile` confirmation summary and accepts a terminal correction before Discovery is invoked, and that correction is reflected in the profile actually used, in `tests/integration/test_confirmation_free_text.py` (FR-006)
- [ ] T040 [P] [US1] Integration test: using `FakeTmdbClient`'s call-count instrumentation (T025), the Discovery Agent's detail-level fetch (runtime for TV, provider list, thematic keywords) is called exactly once per finalist candidate reaching ranking, and zero times for candidates eliminated by hard filtering, for a fixture raw pool larger than the finalist set, in `tests/integration/test_finalist_only_enrichment.py` (FR-029)

### Implementation for User Story 1

- [ ] T041 [US1] Implement the `PreferenceAgent` (PydanticAI, using `ModelProvider`) mapping free text to `PreferenceProfile`, including hard/soft classification and `hard_override_fields` detection for user-designated non-negotiable fields, in `src/streaming_discovery/agents/preference_agent.py`, per contracts/preference-agent.md (depends on T008, T029–T031)
- [ ] T042 [US1] Implement the `DiscoveryAgent`'s single-attempt path — `PreferenceProfile` → `DiscoveryQuery` → `TmdbClient` → normalized `CandidatePool`, with exact-match `exclude_titles` filtering and finalist-only detail enrichment (satisfying T040) — in `src/streaming_discovery/agents/discovery_agent.py`, per contracts/discovery-agent.md (depends on T010, T022–T026)
- [ ] T043 [US1] Implement the `RecommendationAgent`: re-apply hard filters, score soft fit, assign roles in priority order — filling only as many roles as distinct qualifying candidates support, per FR-015 (satisfying T038) — write a rationale per filled role, and exclude near-duplicates via the `vote_average` tie-break (spec.md Assumptions), in `src/streaming_discovery/agents/recommendation_agent.py`, per contracts/recommendation-agent.md (depends on T012, T016, T029–T031)
- [ ] T044 [US1] Wire the Orchestrator's single-attempt path end-to-end (Preference Agent → confirmation → Discovery Agent → Recommendation Agent → assembled `RecommendationPackage`) in `src/streaming_discovery/agents/orchestrator.py` (depends on T033, T041–T043)
- [ ] T045 [US1] Implement the CLI entry point — read one free-text request, render the T034 confirmation summary and accept a terminal correction (satisfying T039 and FR-006 for every request path, not only guided intake), run the Orchestrator, render the `RecommendationPackage` (never raw JSON, per FR-021) — in `src/streaming_discovery/cli/output.py` and a runnable `__main__`/entry-point script (depends on T034, T044)
- [ ] T046 [P] [US1] Unit test: a structured log record is emitted for each agent invocation, each TMDB call, and each contract-validation outcome, in `tests/unit/test_orchestrator_logging.py` (NFR-006) — write this first; it must fail before T047
- [ ] T047 [US1] Add structured logging of agent invocations, TMDB calls, and validation outcomes (satisfying T046, NFR-006) in `src/streaming_discovery/agents/orchestrator.py`
